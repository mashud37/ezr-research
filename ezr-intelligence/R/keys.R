# Service namespace under which all ezrintelligence secrets live in the OS
# keyring.
.ezrintelligence_service <- "ezrintelligence"
...
#' namespaced under the `"ezrintelligence"` service. Keys stored this way are
#' picked up automatically by [ai_chat()] and [ai_summarise()], so secrets never
#' have to live in your scripts.
#'
...
#' `keyring` package, namespaced under the `"ezrintelligence"` service, so they
#' never appear in your scripts or history. [ai_chat()] and [ai_summarise()]
#' look them up automatically (falling back to the conventional environment
#' variable, e.g. `OPENAI_API_KEY`). In an interactive session, omit `key` to be
#' prompted without echoing it to the console.
#'
#' @family keys
...
set_llm_key <- function(provider, key = NULL) {
  require_keyring()
  if (is.null(key)) {
    keyring::key_set(service = .ezrintelligence_service, username = provider)
  } else {
    keyring::key_set_with_value(service = .ezrintelligence_service,
...
#'
#' Looks up a provider's API key, first in the `"ezrintelligence"` keyring
#' service and then in the conventional environment variable (e.g.
#' `OPENAI_API_KEY`).
#'
...
get_llm_key <- function(provider, error = TRUE) {
  ...
  if (requireNamespace("keyring", quietly = TRUE)) {
    key <- tryCatch(
      keyring::key_get(service = .ezrintelligence_service,
                       username = provider),
...
delete_llm_key <- function(provider) {
  require_keyring()
  tryCatch(
    keyring::key_delete(service = .ezrintelligence_service,
                        username = provider),
...
#'
#' @return A character vector of provider names stored under the
#'   `"ezrintelligence"` keyring service (empty if none / keyring unavailable).
#' @family keys
#' @seealso [set_llm_key()].
#' @examples
#' \dontrun{
...
list_llm_keys <- function() {
  ...
  keys <- tryCatch(keyring::key_list(service = .ezrintelligence_service),
