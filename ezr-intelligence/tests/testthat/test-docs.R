...
rd_sections <- function() {
  db <- tryCatch(tools::Rd_db("ezrintelligence"), error = function(e) NULL)
  skip_if(is.null(db), "Rd database not available (package not installed).")
  db
}

exported_function_topics <- function(db) {
  ns <- asNamespace("ezrintelligence")
  exports <- getNamespaceExports("ezrintelligence")
