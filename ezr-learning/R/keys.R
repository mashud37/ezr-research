# Service namespace under which all ezrlearning secrets live in the OS keyring.
.ezrlearning_service <- "ezrlearning"
...
#' namespaced under the `"ezrlearning"` service. Keys stored this way are picked
#' up automatically by [ai_chat()] and [generate_exercise()], so secrets never
...
#' `keyring` package, namespaced under the `"ezrlearning"` service, so they never
#' appear in your scripts or history. The AI helpers look them up automatically
#' (falling back to the conventional environment variable, e.g. `OPENAI_API_KEY`).
#' In an interactive session, omit `key` to be prompted without echoing it.
#'
#' @family ai
...
set_llm_key <- function(provider, key = NULL) {
  require_keyring()
  if (is.null(key)) {
    keyring::key_set(service = .ezrlearning_service, username = provider)
  } else {
    keyring::key_set_with_value(service = .ezrlearning_service,
...
#'
#' Looks up a provider's API key, first in the `"ezrlearning"` keyring service and
...
get_llm_key <- function(provider, error = TRUE) {
  ...
  if (requireNamespace("keyring", quietly = TRUE)) {
    key <- tryCatch(
      keyring::key_get(service = .ezrlearning_service, username = provider),
...
delete_llm_key <- function(provider) {
  require_keyring()
  tryCatch(
    keyring::key_delete(service = .ezrlearning_service, username = provider),
...
#'
#' @return A character vector of provider names stored under the `"ezrlearning"`
...
list_llm_keys <- function() {
  ...
  keys <- tryCatch(keyring::key_list(service = .ezrlearning_service),
