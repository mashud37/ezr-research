...
#' @examples
#' library(ggplot2)
#' p <- ggplot(nps_drivers, aes(value, nps)) + geom_point()
#' \donttest{
...
#' Quick-save any ezrmodel output (plot or table)
#'
...
#'
#' A quick way to drop a handful of results into a single `.xlsx`, one per
#' worksheet -- e.g. `export_xlsx(tidy(drivers(d, y)), tidy(correlations(d)),
#' path = "out.xlsx")`. Tabs are named from the argument names you give, else
#' from each table's first identifying column, else `Sheet1`, `Sheet2`, ...
#'
...
#' export_xlsx(
#'   drivers = tidy(drivers(nps_drivers, nps, methods = "cor")),
#'   correlations = tidy(correlations(nps_drivers, nps)),
#'   path = tmp
#' )
#' file.exists(tmp)
#' @export
