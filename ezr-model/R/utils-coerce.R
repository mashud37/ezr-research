...
#'
#' A defensive helper for the common data-export headache where a column that
...
#' numeric, leaving genuinely unparseable entries as `NA`. The ezrmodel
#' functions that need numbers (e.g. [model_frame()], [correlations()]) call this
...
#' callers ([model_frame()], [correlations()], [cluster()], ...) pass
#' `quiet = TRUE` so they don't spam your console. Truly blank entries
#' (`""`/`NA`) are not counted as parse failures.
#'
#' @family prep
#' @seealso [na_blank()], [model_frame()].
#' @examples
