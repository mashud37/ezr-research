...
test_that("save_data writes xlsx, including multi-sheet lists", {
  ...
  save_data(calc_percentage(consumer_survey, demo_gender), one)
  expect_true(file.exists(one))

  many <- file.path(dir, "many.xlsx")
  save_data(list(gender = calc_percentage(consumer_survey, demo_gender),
                 edu = calc_percentage(consumer_survey, demo_edu)), many)
  expect_true(file.exists(many))
})

test_that("save_data rejects unknown extensions", {
  expect_error(save_data(consumer_survey, tempfile(fileext = ".foo")))
})
...
test_that("save_plot rejects non-ggplot input", {
  expect_error(save_plot(consumer_survey, tempfile(fileext = ".png")))
})

test_that("save_output dispatches on object type", {
  dir <- withr::local_tempdir()
  csv <- file.path(dir, "o.csv")
  save_output(calc_percentage(consumer_survey, demo_gender), csv)
  expect_true(file.exists(csv))

  png <- file.path(dir, "o.png")
  save_output(plot_bars(calc_percentage(consumer_survey, demo_gender)), png)
  expect_true(file.exists(png))
