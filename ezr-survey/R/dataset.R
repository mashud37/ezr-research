...
#' the function runs, a *bare* positional call like `calc_percentage(demo_gender)`
#' cannot fall back to the default -- R would try to treat `demo_gender` as the
#' data. So, with a default set, call the helpers in one of these equivalent
#' ways:
#'
#' * pipe the data in once: `consumer_survey |> calc_percentage(demo_gender)`;
#' * name the column: `calc_percentage(column = demo_gender)`;
...
#' calc_percentage(column = demo_gender)   # data taken from the default
#' calc_nps(value = nps_value)
#' clear_dataset()
#' @export
use_dataset <- function(data) {
  ...
  assign("data", data, envir = .ezrsurvey_active)
  invisible(data)
}
...
get_dataset <- function() {
  ...
  get("data", envir = .ezrsurvey_active, inherits = FALSE)
}

#' @rdname dataset_default
#' @export
has_dataset <- function() {
  exists("data", envir = .ezrsurvey_active, inherits = FALSE)
}
...
clear_dataset <- function() {
  if (has_dataset()) {
    rm("data", envir = .ezrsurvey_active)
  }
  invisible(TRUE)
}
