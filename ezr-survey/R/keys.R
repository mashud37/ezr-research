# Service namespace under which all ezrsurvey secrets live in the OS keyring.
.ezrsurvey_service <- "ezrsurvey"
...
#' namespaced under the `"ezrsurvey"` service. Keys stored this way are picked up
...
#' `keyring` package, namespaced under the `"ezrsurvey"` service, so they never
...
set_llm_key <- function(provider, key = NULL) {
  require_keyring()
  if (is.null(key)) {
    keyring::key_set(service = .ezrsurvey_service, username = provider)
  } else {
    keyring::key_set_with_value(service = .ezrsurvey_service,
...
#'
#' Looks up a provider's API key, first in the `"ezrsurvey"` keyring service and
...
get_llm_key <- function(provider, error = TRUE) {
  ...
  if (requireNamespace("keyring", quietly = TRUE)) {
    key <- tryCatch(
      keyring::key_get(service = .ezrsurvey_service, username = provider),
...
delete_llm_key <- function(provider) {
  require_keyring()
  tryCatch(
    keyring::key_delete(service = .ezrsurvey_service, username = provider),
...
#'
#' @return A character vector of provider names stored under the `"ezrsurvey"`
...
list_llm_keys <- function() {
  ...
  keys <- tryCatch(keyring::key_list(service = .ezrsurvey_service),
